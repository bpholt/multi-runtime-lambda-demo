...
lazy val `node-handler` =
  project
    .in(file("node-lambda"))
    .settings(sharedSettings: _*)
    .settings(
      description := "Node.js Lambda handler wrapper",
      libraryDependencies ++= {
        Seq(
          "com.dwolla" %%% "fs2-aws-lambda-io-app" % fs2AwsV,
        )
    ...
    .dependsOn(`multi-runtime-lambda-demo`.js)
    .enablePlugins(ScalaJSBundlerPlugin)

lazy val `jvm-handler` =
  project
    .in(file("jvm-lambda"))
    .settings(sharedSettings: _*)
    .settings(
      description := "JVM Lambda handler wrapper",
      libraryDependencies ++= {
        Seq(
          "com.dwolla" %%% "fs2-aws-lambda-io-app" % fs2AwsV,
        )
    ...
    .dependsOn(`multi-runtime-lambda-demo`.jvm)
    .enablePlugins(AssemblyPlugin)

lazy val `fargate-handler` = project
  .in(file("fargate-handler"))
  .settings(sharedSettings: _*)
  .settings(
    description := "JVM handler with a standalone HTTP server in a Docker container",
    libraryDependencies ++= {
